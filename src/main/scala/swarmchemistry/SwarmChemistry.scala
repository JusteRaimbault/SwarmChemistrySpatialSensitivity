...
object SwarmChemistry {
  ...
  def initialisePopulation(initMode: String)(implicit model: SwarmChemistry, rng: Random): Unit = {
    ...
    swarmInXOrder = swarmInBirthOrder.clone() // necessary first initialisation before updateStates
    swarmInYOrder = swarmInBirthOrder.clone()
  ...
  def updateStates(implicit model: SwarmChemistry): Unit = {
    ...
    for (i <- 1 until swarmInBirthOrder.length) {
  ...
  object Space {
    ...
    case class ExpMixtureGridGenerator(
      ...
      def generateGrid(implicit rng: Random): Array[Array[Double]] = {
        def kernel(kernelRadius: Double)(x: Double, y: Double): Double = maxValue*math.exp(-math.sqrt(math.pow(x,2.0)+math.pow(y,2.0))/kernelRadius)
        ...
        kernelMixture(size,eithcenters,expKernels,rng)
      }
      ...
      spatialCompetition = mode match {
        ...
        case "zipf" => // hierarchical structure
          ...
          val pr: Array[Array[Double]] =
            ExpMixtureGridGenerator((model.spaceRasterWidth, model.spaceRasterHeight),
